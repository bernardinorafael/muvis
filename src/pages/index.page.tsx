...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const nowPlayingMovies = response.data.results.map((movie: Movie) => {
    return {
      id: movie.id,
...
export default function Home({ nowPlayingMovies }: HomeProps) {
  return (
    <>
      <Head>
      ...
      <section className="flex flex-col gap-10 px-10 py-5">
        ...
        <CarouselCardMovie>
          <div className={cn('grid grid-cols-6 gap-3', 'keen-slider__slide')}>
            {nowPlayingMovies.slice(6, 12).map((movie, i) => {
              return (
